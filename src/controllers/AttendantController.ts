...
export class AttendantController extends Controller<AttendantModel> {
    ...
    public add = async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (attendant[0]) return SendHTTPResponse({ message: 'Este atendente já está cadastrado', type: 'warning', status: true, code: HTTPResponseCode.informationAlreadyExists }, res)
