...
export class ClientController extends Controller<ClientModel> {
    ...
    public add = async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (contact[0]) return SendHTTPResponse({ message: 'Este contato já está em uso', type: 'warning', status: true, code: HTTPResponseCode.informationAlreadyExists }, res)
        ...
        if (conditions.length > 0) {
            ...
            if (client[0]) return SendHTTPResponse({ message: 'Este cliente já está cadastrado', type: 'warning', status: true, code: 409 }, res)
        }
        ...
        try {
            ...
            let contact = await client.getContacts({ where: { value: contato } }); //Search results in Client
            if (contact[0]) return SendHTTPResponse({ message: 'este contato já está em uso', type: 'error', status: false, code: HTTPResponseCode.informationAlreadyExists }, res)

            let contact_c = client.createContact({
                value: contato,
                user_id: user.id
            })

            if (!contact_c) return SendHTTPResponse({ message: 'O cliente foi criado, mas não foi possível adicionar o contato', type: 'warning', status: false, code: HTTPResponseCode.partiallyCompletedProcess }, res)
