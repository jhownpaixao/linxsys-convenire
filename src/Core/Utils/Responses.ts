...
export async function CheckRequest(params: Object) {
  ...
  if (!pass)
    throw new AppProcessError(
      ...
      HTTPResponseCode.incompleteRequest,
      'error',
      { need }
    );
}
...
export function ThrowHTTPErrorResponse(code: number, exception: Error, res: Response) {
  logger.error(
    ...
    'Uma requisição retornou um erro'
  );
  SendHTTPResponse(
    { message: 'Houve um erro ao acessar este recurso', type: 'error', status: false, code: 500 },
    res
  );
}
...
export const handleProcessFailure = async (
  err: Error,
  req: Request,
  res: Response,
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  SendHTTPResponse(
    {
      ...
      type: 'error',
      status: false,
      code: HTTPResponseCode.routeNotFound
    },
    res
  );
};
