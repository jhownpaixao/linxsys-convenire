...
export class ChatService {
    ...
    static async get(id: string | number) {
        ...
        if (!register) throw new AppProcessError('O chat não foi localizado', HTTPResponseCode.informationNotFound);
        return register;
    }
    ...
    static async getMessage(chat_id: string | number, params: WhereOptions<InferAttributes<ChatMessageModel, { omit: never }>>) {
        const chat = await this.get(chat_id);
        if (!chat) throw new AppProcessError('O chat não foi localizado', HTTPResponseCode.informationNotFound);
    ...
    static async hasMessage(chat_id: string | number, assessment_id: string | number) {
        ...
        if (!message) throw new AppProcessError('A mensagem não foi localizado', HTTPResponseCode.informationNotFound);
