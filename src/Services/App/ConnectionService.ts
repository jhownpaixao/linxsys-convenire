...
import type { ConnectionProfilesModel } from '../sequelize/Models';
import { ConnectionModel } from '../sequelize/Models';
...
export class ConnectionService {
  ...
  static async getProfile(id: string | number) {
    ...
    if (!profile)
      throw new AppProcessError(
        'Esta conexão não possui um perfil vinculado',
        HTTPResponseCode.informationNotFound,
        'warning'
      );
  ...
  static async setProfile(
    ...
    if (conn.config_id == profile_id)
      throw new AppProcessError(
        'Este perfil já está vinculado à esta conexão',
        HTTPResponseCode.informationAlreadyExists,
        'warning'
      );
