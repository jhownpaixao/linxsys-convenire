...
export class ConnectionProfileService {
  static async create(
    data: MakeNullishOptional<InferCreationAttributes<ConnectionProfilesModel>>,
    ...
    if (register)
      throw new AppProcessError(
        'Este nome de perfil já está em uso',
  ...
  static async getChatbot(id: string | number) {
    ...
    if (!chatbot)
      throw new AppProcessError(
        'Este perfil não possui um chatbot vinculado',
        HTTPResponseCode.informationNotFound,
        'warning'
      );
    return chatbot;
  ...
  static async setChatbot(
    ...
    if (profile.chatbot_id == chatbot_id)
      throw new AppProcessError(
        'Este chatbot já está vinculado à este perfil',
        HTTPResponseCode.informationAlreadyExists,
        'warning'
      );
