...
import { ConnectionProfileService, ConnectionService, EnvironmentService } from '../services/app';
...
export class ConnectionMiddleware {
  static check = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!(await EnvironmentService.hasConnection(user_id, connection_id)))
  ...
  static checkProfile = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!(await EnvironmentService.hasProfile(user_id, profile_id)))
      throw new AppProcessError(
